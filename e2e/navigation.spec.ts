...
test.describe("navigation and shell", () => {
  test("loads the dashboard at /", async ({ page }) => {
    await page.goto("/");
    // The shared SuiteShell renders the brand as sidebar text (not an <h1>), so
    // assert the Dashboard page's own heading. PageHeader renders it twice (the
    // desktop top-bar copy + a mobile fallback), so scope to the first match.
  ...
  test("HashRouter navigates between top-level pages", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: "Import Excel" }).first()).toBeVisible();
  });
